$(document).ready(function () {
    ...
    $(".ativado").click(function () {
        ...
        if (primeiro.data('id') === segundo.data('id')) {
            ...
            if (contagem === 8) {
                ...
                var victorySound = new Audio("http://localhost/MemoriaMaluquinha/audios/vitoria.mp3");
                victorySound.play();
    ...
    $(".opc").click(function () {
        if ($(this).data("id") === 1) {
            $('#clickSound').attr('src', "http://localhost/MemoriaMaluquinha/audios/som1.mp3")[0].play();
        }
        if ($(this).data("id") === 2) {
            $('#clickSound').attr('src', "http://localhost/MemoriaMaluquinha/audios/som2.mp3")[0].play();
        }
        if ($(this).data("id") === 3) {
            $('#clickSound').attr('src', "http://localhost/MemoriaMaluquinha/audios/som3.mp3")[0].play();
        }
        if ($(this).data("id") === 4) {
            $('#clickSound').attr('src', "http://localhost/MemoriaMaluquinha/audios/som4.mp3")[0].play();
        }
    });
    ...
    $("#finalizar").click(function () {
        if (confirm("Tem certeza de que deseja limpar o jogo?")) {
            fetch('http://localhost/MemoriaMaluquinha/php/limpar_jogo.php', { method: 'POST' })
                .then(response => response.text())
                .then(data => {
                    alert(data); // Mostra a mensagem do PHP
                    window.location.href = "http://localhost/MemoriaMaluquinha/escolherTema.html"; // Redireciona para a seleção de tema
                })
    ...
    $("#btnCriarNovoJogo").click(function () {
        if (confirm("Tem certeza de que deseja criar um novo jogo?")) {
            fetch('http://localhost/MemoriaMaluquinha/php/limpar_jogo.php', { method: 'POST' })
                ...
                .then(data => {
                    alert(data); // Mostra a mensagem do PHP
                    window.location.href = "http://localhost/MemoriaMaluquinha/escolherTema.html"; // Redireciona para seleção de tema
                })
